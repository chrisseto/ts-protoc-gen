...
function generateTypescriptDefinition(fileDescriptor, exportMap) {
    ...
    codePrinter.printLn("export interface FromObject {")
        .indent();
    var imp = serviceDescriptor.imports[0];
    var impNamespace = imp.namespace;
    fileDescriptor.getMessageTypeList().forEach(function (messageDescriptor) {
        var msgName = messageDescriptor.getName();
...
function generateJavaScript(fileDescriptor, exportMap) {
    ...
    serviceDescriptor.imports.forEach(function (importDesc) {
...
function printFromObj(printer, namespace, message) {
    var codePrinter = new CodePrinter_1.CodePrinter(0, printer);
    ...
    message.getFieldList().forEach(function (fieldDesc) {
        var fieldName = fieldDesc.getName();
        var snakeCaseName = fieldName.toLowerCase();
        var camelCaseName = util_1.snakeToCamel(snakeCaseName);
        var withUppercase = util_1.uppercaseFirst(camelCaseName);
        if (fieldDesc.getType() === descriptor_pb_1.FieldDescriptorProto.Type.TYPE_MESSAGE) {
